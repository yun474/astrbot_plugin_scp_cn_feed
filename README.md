...
在聊天中执行 `/scpfeed 订阅` 或 `/scpfeed 取消` 后，插件会立刻把结果写回 `subscription_sessions`，方便之后在配置页统一删改。直接在配置页修改订阅会话后，保存并重载插件即可同步到运行状态。
...
- 更新推送形式对自动轮询和 `/scpfeed 检查` 同时生效；截图或日报图片渲染失败时会自动回退文字。
